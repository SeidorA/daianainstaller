...
CREATE FUNCTION public.reserve_tenant_message_quota(integer, text, text, text, text, text, timestamptz)
...
SQL
  ...
  cp "$ROOT_DIR/volumes/db/daiana-migrations/20260824160000_harden_replay_quota_rpc_acl.sql" "$migration_dir/"
  ...
  checks="$(docker exec -e PGPASSWORD=test-password "$container" psql -U postgres -d postgres -Atqc "
    ...
    SELECT has_function_privilege('service_role', 'public.finalize_tenant_message_quota_turn(text,text,jsonb,jsonb,timestamptz)', 'EXECUTE');
     ...
     SELECT NOT has_function_privilege('anon', 'public.reserve_tenant_message_quota(integer,text,text,text,text,text,timestamptz)', 'EXECUTE');")"
  [ "$(printf '%s\n' "$checks" | grep -c '^t$')" -eq 9 ] \
