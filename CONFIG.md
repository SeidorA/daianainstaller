...
## Daiana configuration ownership

The installer keeps bootstrap, internal, and secret values in `.env`. Vault is the runtime source of truth for public URL values consumed by the Daiana frontend through its EnvProvider. `volumes/db/init/vault.sql` upserts these public URL keys during installation:

- `NEXT_PUBLIC_SUPABASE_URL`
- `NEXT_PUBLIC_API_PYTHON`
- `NEXT_PUBLIC_API_TRAINING`
- `NEXT_PUBLIC_API_QDRANT`
- `NEXT_PUBLIC_API_MSTEAMS`
- `NEXT_PUBLIC_API_WHATSAPP`
- `NEXT_PUBLIC_API_STUDIO_BASE_URL`
- `NEXT_PUBLIC_WEBUI_URL`
- `NEXT_PUBLIC_APP_URL`

`NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY` remain in the Compose bootstrap environment because they are required to initialize and access Supabase/Vault. The other Vault-backed public URL keys are intentionally not passed from `docker-compose.app.yml` into `daiananext`; the frontend resolves them from Vault.

The following values remain installer-owned in `.env` and are passed directly where needed:

- `STUDIO_PROVISIONING_URL` defaults to `http://daianastudio:3000` and is internal server-to-server configuration.
- `PRIVATE_CHAT_PYTHON_ORIGIN` is the internal proxy origin used by `daiananext`.
- `STUDIO_PROVISIONING_SECRET` and `TEAMS_INTERNAL_AUTH_SECRET` are generated idempotently and retained in `.env`; the Teams secret is wired to both Python and Teams.
- `DAIANA_SERVER_TELEMETRY_WEBHOOK_URL` is optional and is never generated by the installer.

`NEXT_PUBLIC_HELP_CENTER` is deprecated and unused; it is not wired into the standard Compose configuration.
